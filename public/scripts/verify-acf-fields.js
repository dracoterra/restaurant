 ...
 * Ejecutar con: node scripts/verify-acf-fields.js
 */
...
async function verifyBackendEndpoint() {
  logSection('3. VERIFICANDO ENDPOINT DEL BACKEND');
  
  try {
    const axios = require('axios');
    const response = await axios.get('http://localhost:3030/debug', {
      params: { action: 'status' },
      timeout: 5000
    });
    
    log('✅ Backend respondiendo', 'green');
    log(`   WordPress GraphQL: ${response.data.wordpress?.graphql?.connected ? '✅ Conectado' : '❌ Desconectado'}`, 
        response.data.wordpress?.graphql?.connected ? 'green' : 'red');
    log(`   WordPress REST: ${response.data.wordpress?.rest?.connected ? '✅ Conectado' : '❌ Desconectado'}`, 
        response.data.wordpress?.rest?.connected ? 'green' : 'red');
    
    return true;
  } catch (error) {
    log('⚠️  Backend no disponible (esto es normal si no está corriendo)', 'yellow');
